...
console.log(1234);
console.log("문자열은 양쪽에 쌍따음표 작성");
console.log('JS는 홑따음표도 문자열로 취급');
...
// 2) 괄호 내부에 , 를 작성하는 방법
//   -> 각각 값을 독립적으로 출력 형변환 없이 출력되어 문자열 따로,
...
// 이전에 저장해놓은 변수값을 쓸 수 없게 되는 문제 발생
...
// const test; 상수는 선언과 동시에 초가화 반드시!!
...
// let과 const로 선언된 변수는 변수가 선언된 블록(중괄호{})
// 내부에서만 유효하다는 것을 의미
// 즉, 코드블록 내부에서 선언한 변수(let,const)는 해당 블록
// 외부에서 접근할 수 없다
...
console.log(test2);
// -> test2 도 출력 가능
// -> 왜? 함수 레벨 스코프인 var는 일반 블록은 무시하기 떄문
...
// 1. var의 호이스팅
// : var로 선언된 변수는 선언만 해당 스코프으 최상단으로 호이스팅 됨.
//   초기화는 원래 코드 위치에서 수행됨

// 에러가 안나고 undefinded 가 찍힘
// -> 호이스팅 되어 var a; 선언부는 최상단으로 올라감.
// -> 선언은 되었으나 a에 값이 대입되진 않았기 때문에 undefinded 가 되는 것

// 내부적으로 var a; 이렇게 선언만 된 상태
console.log(a); // undefinded

var a = 10; // 전역 스코프에 선언됨 + 선언과 동시에 초기화

console.log(a); 

// 2. let/const 의 호이스팅
// : let/const 로 선언된 변수는 선언만 해당 스코프의 최상단으로 호이스팅됨
//   var 와 달리, let/const 로 선언된 변수는 
...
// 초기화되기 전까지 접급할 수 없는 구간을 말함

// 내부적으로 let x; 선언은 끌어올려진 상태
// -> x의 선언이 호이스팅됨.
// 그러나 TDZ에 놓여 초기화 전까지 접근 불가
// -> 초기화가 이루어지기 전까지는 해당 변수는 TDZ라는 곳에 놓이며,
// 이 기간동안 변수에 접근하려고 하면
...
// -> 이러한 특성들 때문에 let/const 는 var보다는 
...
console.log("isTrue : " , true , typeof true);
console.log("isfalse : " , false , typeof false);
...
console.log("numbers 베열 값 중 0번째 : " , numbers[0] , typeof numbers[0]);
console.log("numbers 베열 값 중 1번째 : " , numbers[1] , typeof numbers[1]);
console.log("numbers 베열 값 중 2번째 : " , numbers[2] , typeof numbers[2]);
console.log("numbers 베열 값 중 3번째 : " , numbers[3] , typeof numbers[3]);

// JS 에서의 배열은 값 추가 및 삭제가 가능
...
// 방법 2 ) 변수명.key
console.log(user.id,user.pw,user.userName,user.age); // user01 pass01 홍길동 20
