...
const bth1 = document.querySelector("#btn1");
...
// 매개 변수로 전달받은 배열의 모든 요소를 콘솔에 출력하는 함수
...
document.querySelector("#btn2a").addEventListener("click",function(){
  ...
  console.log(x);
  // x 는 원시타입으로 값 자체를 전달할 뿐, 주소값을 전달하는게 아니기 때문에
  // 복사본의 값이 변경되도 원본의 값은 변경되지 않음

  // JS 원시타입종류
  // string / number / boolean / undefined / null ..
  // 원시타입 : 객체가 아니라, 값 자체를 지정(원본 불변)
  
  // JS 객체타입종류
  //  odject / Array / Function..
  // 참조형(객체 타입) : 메모리에 저장된 주소를 참조함(복사본이 변경되면, 원본도 변함)
...
document.querySelector("#btn2c").addEventListener("click",function(){
  ...
  // 함수도 변수에 저장 가능함 (변수명 == 함수명)
  const sumFn = function(a,b){
    return a+b; // 함수를 즉시 종료하는 동시에 옆에 작성된 값을 가지고 호출한 곳으로 돌아감
  } 
...
arrowList[1].addEventListener("click", e => {
  ...
  // 화살표 함수에서의 this 이벤트가 일어난 대상 객체가 아니라
  // 전역 객체 (window)
