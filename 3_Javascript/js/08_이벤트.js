...
// 아이디가 test1-1 요소의 onclick 이벤트 리스너의
// 이벤트 핸들러 제거하기
document.querySelector("#test1-2").onclick =
function() {
  // 기존 onclick의 이벤트핸들러(함수)를 null로 덮어씌움 (이벤트 제거)
  test1a.onclick = null;
...
// onclick에 저장된 값(이벤트 핸들러 함수)이 덮어씌워지면서 
...
test2.addEventListener("click",function(){
  ...
  if(test2.style.opacity == 0){ // 완전히 투명해짐
    test2.style.opacity = 1; // 다시 불투명해짐
  }
});
