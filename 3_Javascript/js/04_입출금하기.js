...
// 금액 작성 input요소 -> <input type="text" id="amount">

const output = document.getElementById("output");
// 잔액을 출력해주는 span 요소 ->  <span id="output"></span>

let balance = 10000;       // 잔액 기록 변수 (초기값 10000)

const pw = '1234';    // 비밀번호 저장 변수(초기 비밀번호 1234)
...
function deposit(){
  ...
  // amount.value.length == 0 --> 입력된 값이 없다
  if(amount.value.length == 0) {
    alert("금액을 입력해주세요!!");

  }else{ // 입력이 되었다면

    // 구한 금액을 잔액 (balance)에 누적 구한 금액(amount.value)이 현재 string
